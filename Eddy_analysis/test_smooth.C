...
void test_smooth(TString input_file, TString output_file=""){
	...
	TH1D* trace = ((TProfile*)f->Get("trace_calibrated"))->ProjectionX();
	TH1D** kicks = new TH1D*[8];
	for (int i=0; i<8; i++){
		kicks[i] = ((TProfile*)f->Get(Form("trace_kick%d_calibrated",i+1)))->ProjectionX();
	}
	TH1D* kick8long = ((TProfile*)f->Get("trace_kick8long"))->ProjectionX();
	...
	for (int i=0; i<8; i++){
		...
		h1_fft_kicks[i]->Scale(1./h1_fft_kicks[i]->Interpolate(0.2));
	}
	for (int i=0; i<8; i++){
		h1_fft_kicks_smooth[i] = doFFT(kicks_smooth[i],fft_xmin,fft_xmax);
		h1_fft_kicks_smooth[i]->GetYaxis()->SetRangeUser(0,0.01);
		h1_fft_kicks_smooth[i]->Scale(1./h1_fft_kicks_smooth[i]->Interpolate(0.2));
	}
	for (int i=0; i<8; i++){
		h1_fft_kicks_rebin[i] = doFFT(kicks_rebin[i],fft_xmin,fft_xmax);
		h1_fft_kicks_rebin[i]->GetYaxis()->SetRangeUser(0,0.01);
		h1_fft_kicks_rebin[i]->Scale(1./h1_fft_kicks_rebin[i]->Interpolate(0.2));
	}
	for (int i=0; i<8; i++){
		h1_fft_kicks_lowpass[i] = doFFT(kicks_lowpass[i],fft_xmin,fft_xmax);
		h1_fft_kicks_lowpass[i]->GetYaxis()->SetRangeUser(0,0.01);
		h1_fft_kicks_lowpass[i]->Scale(1./h1_fft_kicks_lowpass[i]->Interpolate(0.2));
	}
	for (int i=0; i<8; i++){
		h1_fft_kicks_ra51015[i] = doFFT(kicks_ra51015[i],fft_xmin,fft_xmax);
		h1_fft_kicks_ra51015[i]->GetYaxis()->SetRangeUser(0,0.01);
		h1_fft_kicks_ra51015[i]->Scale(1./h1_fft_kicks_ra51015[i]->Interpolate(0.2));
	}
	...
	TCanvas* can_compare = new TCanvas("can_compare");
	can_compare->Divide(2,1);
	can_compare->cd(1);
	kick1_zoom->SetTitle("Original");
	kick1_zoom_smooth->SetTitle("Smooth()");
	kick1_zoom_rebin->SetTitle("Rebin(10)");
	kick1_zoom_lowpass->SetTitle("Lowpass 50 kHz");
	kick1_zoom_ra51015->SetTitle("Kernel 5-10-15");
	kick1_zoom->Draw("HIST");
	kick1_zoom_smooth->Draw("HIST SAME");
	kick1_zoom_rebin->Draw("HIST SAME");
	kick1_zoom_lowpass->Draw("HIST SAME");
	...
	gPad->BuildLegend();
	can_compare->cd(2);
	h1_fft_kicks[0]->SetLineColor(kBlue);
	h1_fft_kicks_smooth[0]->SetLineColor(kRed);
	h1_fft_kicks_rebin[0]->SetLineColor(kOrange);
	h1_fft_kicks_lowpass[0]->SetLineColor(kGreen+2);
	h1_fft_kicks_ra51015[0]->SetLineColor(kViolet);
	h1_fft_kicks[0]->SetLineWidth(2);
	h1_fft_kicks_smooth[0]->SetLineWidth(2);
	h1_fft_kicks_rebin[0]->SetLineWidth(2);
	h1_fft_kicks_lowpass[0]->SetLineWidth(2);
	h1_fft_kicks_ra51015[0]->SetLineWidth(2);
	h1_fft_kicks[0]->Draw("HIST");
	h1_fft_kicks_smooth[0]->Draw("HIST SAME");
	h1_fft_kicks_rebin[0]->Draw("HIST SAME");
	h1_fft_kicks_lowpass[0]->Draw("HIST SAME");
	h1_fft_kicks_ra51015[0]->Draw("HIST SAME");
